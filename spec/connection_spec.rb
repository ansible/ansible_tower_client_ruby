describe AnsibleTowerClient::Connection do
  let(:base_options) { {:base_url => "example1.com", :username => "admin", :password => "smartvm", :verify_ssl => OpenSSL::SSL::VERIFY_NONE} }
  ...
  it ".new doesn't cache credentials across all instances" do
    ...
    conn_2 = described_class.new(base_options.merge(:base_url => "example2.com", :username => "user", :password => "password"))
